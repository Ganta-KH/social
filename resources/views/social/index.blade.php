...
@section('content')
    <div class="p-info navbar navbar-expand-md navbar-light bg-white shadow-sm">
        <img src="{{ asset($user->avatar) }}" alt="">
        <div class="p-details">
            <div class="username-edit">
                <h1><strong>{{ $user->username }}</strong></h1>
                <form action="" method="get">
                    <input type="submit" name="edit-p" class="edit-p" value="Edit profile">
                </form>
            </div>
            <div class="details">
                <div class="numbers">
                    <h5>{{ $posts->count() }} <strong>posts</strong></h5>
                    <h5>{{ $posts->count() }} <strong>followers</strong></h5>
                    <h5>{{ $posts->count() }} <strong>following</strong></h5>
                </div>
                <p>description</p>
            </div>
        </div>
